...
if(!/vingt heures réparties sur trois semaines/.test(historyFrMd)||!/twenty hours spread over three weeks/.test(historyEnMd)||!/une seule conversation, volontairement conservée sans compaction/.test(historyFrMd)||!/one single conversation, deliberately kept uncompacted/.test(historyEnMd)||!/≈ 330 M/.test(historyFrMd)||!/≈ 330 M/.test(historyEnMd)||!/245 à 250 \$/.test(historyFrMd)||!/\$245–250/.test(historyEnMd))throw new Error("The bilingual histories do not document the certain single-conversation setup and the supplied time, token, and API-equivalent estimates.");
if(!/500 \$ pour le fermier/.test(historyFrMd)||!/\$500 for the farmer/.test(historyEnMd))throw new Error("Bilingual project histories do not contain the current starting funds.");
